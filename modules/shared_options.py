...
options_templates.update(
    options_section(
        ("upscaling", "Upscaling", "postprocessing"),
        {
            ...
            "upscaler_for_img2img": OptionInfo("None", "Upscaler for img2img", gr.Dropdown, lambda: {"choices": upscaler_choices(shared.sd_upscalers)}).info("for resizing the input image if the image resolution is smaller than the generation resolution"),
...
options_templates.update(
    options_section(
        ...
        {
            "localization": OptionInfo("zh_Hant", "Localization", gr.Dropdown, lambda: {"choices": ["None", *localization.localizations.keys()]}, refresh=lambda: localization.list_localizations(cmd_opts.localizations_dir)).needs_reload_ui(),
